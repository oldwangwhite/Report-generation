...
def test_normal_user_can_read_reference_resources_only(client, auth_headers):
    for url in ["/api/templates", "/api/materials"]:
        response = client.get(url, headers=auth_headers).json()
        assert response["code"] == 200
        assert "items" in response["data"]

    for url in ["/api/admin/model-config", "/api/admin/users"]:
        response = client.get(url, headers=auth_headers).json()
